...
import { UserModel, RoleModel, VicedeanModel, SecretaryModel } from '../models/system/index.js';
...
async function seedSecretaryWithFaculty() {
    try {
        console.log('Connecting to MongoDB...');
        await mongoose.connect(MONGO_URI);
        console.log('Connected.');
        ...
        const secretaryRole = await RoleModel.findOne({ name: 'secretary' });
        if (!secretaryRole) {
            console.error("Error: Role 'secretary' not found.");
            return;
        }

        const identification = '10000000000';
        let user = await UserModel.findOne({ identification });
        ...
        const secretaryEntry = await SecretaryModel.findOneAndUpdate(
            { userId: user._id }, 
            { 
                userId: user._id, 
                facultyId: faculty._id 
        ...
        console.log(`Success: Secretary linked to faculty: ${faculty.name}`);
...
await seedSecretaryWithFaculty()
